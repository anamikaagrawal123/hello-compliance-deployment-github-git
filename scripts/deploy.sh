...
sed -i "s~^\([[:blank:]]*\)image:.*$~\1image: ${IMAGE}~" deployment.yaml

deployment_name=$(yq r deployment.yaml metadata.name)
service_name=$(yq r -d1 deployment.yaml metadata.name)

kubectl apply --namespace "$IBMCLOUD_IKS_CLUSTER_NAMESPACE" -f deployment.yaml
